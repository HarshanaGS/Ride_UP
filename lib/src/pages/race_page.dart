...
class _CorridaPageState extends State<CorridaPage> {
  ...
  Future<void> first_root(String requestId) async {
    try {
      // 1. Reference the `requests` collection
      DocumentReference docRef =
      FirebaseFirestore.instance.collection('requests').doc(requestId);
      ...
      if (snapshot.exists) {
        ...
        if (data != null) {
          ...
          if (data['passenger'] != null) {
            destinationLat = data['passenger']['latitude'];
            destinationLng = data['passenger']['longitude'];
          }
  ...
  void _adicionarListenerRequisicao() {
    ...
    _requisicaoStream = db
      ...
      if (snapshot.data() != null) {
        ...
        // ✅ Add second passenger marker here
        if (data_request != null && data_request!.containsKey('second_passenger')) {
          var p2 = data_request!['second_passenger'];
          Marker secondPassengerMarker = Marker(
            markerId: MarkerId('second-passenger'),
            ...
            infoWindow: InfoWindow(
              ...
              onTap: () {
                ...
                Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (context) =>
                        GoogleMapWebView(googleMapsUrl: googleMapsUrl),
                  ),
                );
              },
            ),
  ...
  void _statusUberEmtrip() async {
    ...
    // ✅ Add second passenger marker if available
    if (data_request != null && data_request!.containsKey('second_passenger')) {
      var p2 = data_request!['second_passenger'];
      Marker secondPassengerMarker = Marker(
        markerId: MarkerId('second-passenger'),
        position: LatLng(p2['latitude'], p2['longitude']),
        icon: BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueOrange),
        infoWindow: InfoWindow(title: '2nd Passenger'),
      );
  ...
  void _aceitarCorrida() async {
    ...
    try {
      ...
      // ✅ Show Google Maps WebView with directions
    first_root(RequestD);
  ...
  void _statusUberfinished() async {
    String requestId = data_request!['id']; // Make sure you have this ID
    ...
    if (requestSnapshot.exists && requestSnapshot.data() != null) {
      ...
      estimatedPrice = requestData['estimatedPrice']?.toDouble() ?? 0.0;
    }

    var f = NumberFormat("#,##0.00", "en_US");
    var valortripFormatado = f.format(estimatedPrice);
